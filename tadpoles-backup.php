...
<?PHP
	...
	$absolute_destination_folder = '/home/username/tadpoles'; // Path to a folder where the downloaded items will be stored.
	...
	// Also, if exiftool is installed, set the exif date and convert any png files to jpg.
			...
			if($e->type == 'Activity') {
				foreach($e->new_attachments as $a) {
					...
					if(isset($exiftool_path)) {
						set_exif_date_convert_to_jpg($filename);
					}
	...
	function set_exif_date_convert_to_jpg($filename)
	{
		// Set the file's exif date.
		echo "Setting EXIF date of $filename\n";
		$results = shell_exec("exiftool '-datetimeoriginal<filename' '$filename' 2>&1 1> /dev/null");

		// Sometimes, Tadpoles sends us jpg's that are really png files. If we detect this, convert them.
		if(strpos($results, 'looks more like a PNG') !== false) {
			$mogrify_path = shell_exec('which mogrify');
			if(strlen($mogrify_path) == 0) {
				echo "Error: mogrify not installed. Cannot convert file to JPG.\n";
			} else {
				echo "Converting $filename to JPG\n";
				$results = shell_exec("mogrify -format jpg '$filename'");
				$results = shell_exec("exiftool '-datetimeoriginal<filename' '$filename'");
			}
		}
		
		// exiftool makes a duplicate of the original file when setting the exif info, let's delete it.
